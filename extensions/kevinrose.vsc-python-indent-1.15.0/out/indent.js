...
function editsToMake(lines, currentLine, tabSize, lineNum, charNum, trimLinesWithOnlyWhitespace, keepHangingBracketOnLine) {
    let { nextIndentationLevel: indent } = (0, python_indent_parser_1.indentationInfo)(lines, tabSize);
    let deletes = [];
    ...
    let hanging = (0, python_indent_parser_1.shouldHang)(currentLine, charNum);
    ...
    if (hanging === python_indent_parser_1.Hanging.Partial) {
        toInsert = '\n' + ' '.repeat((0, python_indent_parser_1.indentationLevel)(currentLine) + tabSize);
    }
    else {
        ...
        for (const keyword of Object.keys(dedentKeywords).filter((key) => trimmed.startsWith(key))) {
            for (const matchedLine of lines.slice(1).filter((l) => l.trim().endsWith(":"))) {
                ...
                if (dedentKeywords[keyword].some((matcher) => matchedLineTrimmed.startsWith(matcher))) {
                    const currentIndent = (0, python_indent_parser_1.indentationLevel)(line);
                    const matchedIndent = (0, python_indent_parser_1.indentationLevel)(matchedLine);
